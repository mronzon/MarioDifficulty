...
void inline create_graph(const points_array& points)
{
    ...
    float max_value = 0; // The maximum value of the points.
    ...
    for (points_array::size_type i = 1; i <= n; i++)
    {
        ...
        cv::rectangle(graph_image, rect_horizontal, cv::Scalar(255), cv::FILLED); // Ne pas oublier que c'est que 10 qu'on écrit, pas tous.
        if (i == 10)
        {
            cv::putText(graph_image, std::to_string(i), cv::Point(95 + i*step, 925), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255), 1.5);
        }
        else
        {
            cv::putText(graph_image, std::to_string(i), cv::Point(100 + i*step, 925), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255), 1.5);
        }
